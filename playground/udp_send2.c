...
int main(void)
{
   ...
   // myaddr.sin_addr.s_addr = htonl(INADDR_ANY);
    if (inet_aton("128.101.37.9", &myaddr.sin_addr)==0) {
        fprintf(stderr, "inet_aton() failed\n");
        exit(1);
    }
    myaddr.sin_port = htons(1925);
  ...
  /*      printf("Sending packet %d to %s port %d\n", i, server, SERVICE_PORT);
        ...
        if (sendto(fd, buf, strlen(buf), 0, (struct sockaddr *)&remaddr, slen)==-1)
            perror("sendto");
   */     printf("Receiving from %s on port %d\n", server, 1925);
        fflush(stdout);
	int j;
        if((i = recvfrom(fd, buf, BUFLEN, 0, (struct sockaddr *)&dirt, &addrlen)) == -1){
